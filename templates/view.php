...
(function() {
    ...
    // Sandboxed iframe Email Body logic
    var shadowHost = document.getElementById('email-body-shadow');
    if (shadowHost && <?php echo $hasHtml ? 'true' : 'false'; ?>) {
        var iframe = document.createElement('iframe');
        // Keep sandbox strict (scripts and forms stay blocked; do not loosen allow-same-origin to keep isolation)
        iframe.setAttribute('sandbox', '');
        iframe.setAttribute('referrerpolicy', 'no-referrer');
        iframe.setAttribute('title', 'Email content');
        iframe.style.width = '100%';
        iframe.style.minHeight = '200px'; // avoids layout jump while content loads; adjust if UI needs differ
