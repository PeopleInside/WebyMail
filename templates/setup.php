        ...
        <div class="wm-card">
            ...
            <div class="wm-card-body">
                <form method="post" action="?action=setup">
                    ...
                    <fieldset style="border:1px solid var(--wm-border);border-radius:8px;padding:1rem;margin-bottom:1rem;margin-top:1rem">
                        <legend style="font-size:.82rem;font-weight:600;padding:0 .5rem;color:var(--wm-text-muted)">Appearance &amp; Customisation</legend>

                        <div class="form-group">
                            <label>Interface name
                                <span style="font-size:.75rem;color:var(--wm-text-muted)">(overrides Application name in the UI; leave blank to use Application name)</span>
                            </label>
                            <input type="text" name="interface_name" class="form-control"
                                   placeholder="WebyMail"
                                   value="<?= htmlspecialchars($_POST['interface_name'] ?? AppConfig::get('interfaceName', '')) ?>">
                        </div>

                        <div class="form-group">
                            <label>Favicon path
                                <span style="font-size:.75rem;color:var(--wm-text-muted)">(web-root-relative path to a .ico or .svg file)</span>
                            </label>
                            <input type="text" name="favicon_path" class="form-control"
                                   placeholder="assets/favicon.ico"
                                   value="<?= htmlspecialchars($_POST['favicon_path'] ?? AppConfig::get('faviconPath', '')) ?>">
                        </div>

                        <div class="form-group">
                            <label>Custom footer text
                                <span style="font-size:.75rem;color:var(--wm-text-muted)">(trusted HTML appended below the default footer)</span>
                            </label>
                            <input type="text" name="custom_footer_text" class="form-control"
                                   placeholder="Custom footer HTML"
                                   value="<?= htmlspecialchars($_POST['custom_footer_text'] ?? AppConfig::get('customFooterText', '')) ?>">
                            <p class="form-hint" style="font-size:.78rem;color:var(--wm-text-muted);margin:.35rem 0 0">
                                HTML link example: <code>&lt;a href="https://example.com" target="_blank"&gt;Custom text link&lt;/a&gt;</code>
                            </p>
                        </div>
                    </fieldset>

                    <fieldset style="border:1px solid var(--wm-border);border-radius:8px;padding:1rem;margin-bottom:1rem">
                        <legend style="font-size:.82rem;font-weight:600;padding:0 .5rem;color:var(--wm-text-muted)">Behaviour</legend>

                        <div class="form-group">
                            <label>Timezone <span style="font-size:.75rem;color:var(--wm-text-muted)">(default: Europe/Rome)</span></label>
                            <select name="timezone" class="form-control">
                                <?php
                                $selectedTz = $_POST['timezone'] ?? AppConfig::get('timezone', 'Europe/Rome');
                                foreach (timezone_identifiers_list() as $tz):
                                ?>
                                <option value="<?= htmlspecialchars($tz) ?>"<?= $tz === $selectedTz ? ' selected' : '' ?>>
                                    <?= htmlspecialchars($tz) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <label style="font-size:.85rem;color:var(--wm-text-muted);display:block;margin-bottom:.5rem">
                            <input type="checkbox" name="hide_login_options" value="1"
                                   <?= !empty($_POST['hide_login_options']) || (!isset($_POST['step']) && AppConfig::get('hideLoginOptions', false)) ? 'checked' : '' ?>>
                            Hide ▸ Server settings on the login page
                        </label>

                        <label style="font-size:.85rem;color:var(--wm-text-muted);display:block">
                            <input type="checkbox" name="two_factor_for_all" value="1"
                                   <?= !empty($_POST['two_factor_for_all']) || (!isset($_POST['step']) && AppConfig::get('twoFactorForAll', false)) ? 'checked' : '' ?>>
